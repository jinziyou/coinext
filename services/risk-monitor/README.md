...
The first line is the per-order `qv-risk-engine` gate *inside* each trading node's core; this service
...
| Limits      | `VQ__RISK__*` (shared with the in-core gate — `.env.example`) |
| Bus         | `VQ__REDIS__URL`                                             |

Planned metrics on `:9104`: `vq_risk_drawdown_pct`, `vq_risk_gross_exposure`, `vq_risk_net_exposure`,
`vq_risk_loss_of_day`, `vq_risk_killswitch_trips_total`.

## Run (docker)

```bash
docker build -f deploy/docker/risk-monitor.Dockerfile -t veloxquant/risk-monitor .
docker run --rm -p 9104:9104 \
  -e VQ__REDIS__URL=redis://redis:6379/0 \
  -e VQ__RISK__MAX_GROSS_EXPOSURE=1000000 \
  -e VQ__RISK__MAX_LOSS_OF_DAY=50000 \
  veloxquant/risk-monitor
```

The limit-evaluation core (`RiskSupervisor` / `RiskLimits` / `AccountState`) is pure (no I/O) and
unit-testable without the bus or a running Redis. `qv_bus` and `prometheus_client` are imported
lazily; without them the process runs in an idle stub mode so `/metrics` stays scrapeable.

## TODOs

- Wire the real `qv_bus` async consumer of the telemetry stream + `Envelope` decode.
