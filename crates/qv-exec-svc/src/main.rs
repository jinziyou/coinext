//! `qv-exec-svc` — the standalone **OMS/execution service** (service name `exec-svc`).
//!
//! Per the architecture, this hosts the `qv-exec-engine` (OMS: risk-gated routing, FSM driving,
//! report folding — §3) and `qv-risk-engine` (pre-trade gate + atomic kill-switch) in front of a
//! live `ExecutionClient` (the `qv-adapters-binance` `BinanceExecutionClient`). It is the live
//! counterpart of what the Kernel wires in-process for backtest, behind the SAME `qv-ports` seam, so
...
//!   - config via the `VQ__SECTION__KEY` env convention.
//!
...
async fn main() {
    ...
    println!("=========================================================");
    println!("  VeloxQuant exec-svc (qv-exec-svc)  [SCAFFOLD]");
    ...
    println!("  venue   : Binance (live ExecutionClient behind qv-ports)");
    println!("=========================================================");

    // TODO(exec-svc): the OMS/risk service loop —
    //   1. Load config (VQ__BINANCE__*, VQ__REDIS__URL, RiskLimits) via qv_config.
    //   2. Start the Prometheus exporter (:9102), OTel tracing, and the control server (:8081).
    //   3. Build the RiskEngine (qv-risk-engine) with configured RiskLimits + kill-switch, the OMS
    //      (qv-exec-engine), the persisted SeqCursor + EventStore (qv-persistence), and the live
    //      BinanceExecutionClient (qv-adapters-binance); connect() it and take_reports().
    ...
    eprintln!("qv-exec-svc: scaffold only — OMS/risk loop not yet implemented; exiting.");
}
