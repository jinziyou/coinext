//! `qv-network` — the shared WS/REST framework that every venue adapter builds on.
//!
//! Per the architecture (`docs/ARCHITECTURE.md` §3, the `qv-network` crate), this is the common
//! plumbing that the `qv-adapters/*` venue adapters reuse so each adapter only has to encode the
