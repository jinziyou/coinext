//! `qv-persistence` — the audit-trail + crash-recovery source of truth behind the live runtime.
//!
...
//!
//! [`ParquetWriter`] is the data-lake seam (§7): it materializes normalized [`qv_model::Bar`]s as
//! Arrow/Parquet so the `qv_data` catalog/HistoryReader can serve warm-up identically in backtest
