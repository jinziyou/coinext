...
//! files the `ingestor` writes are read back by the `qv_data` catalog/HistoryReader, so warm-up is
...
impl ParquetWriter {
    ...
    /// a test/inspection helper. The lake's authoritative reader is the `qv_data` catalog.
...
mod tests {
    ...
    fn parquet_bars_roundtrip_close_prices() {
        ...
        let path = dir.join(format!("qv_bars_{}.parquet", std::process::id()));
