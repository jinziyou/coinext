//! [`BinanceInstrumentProvider`] — `impl qv_ports::InstrumentProvider`.
//!
//! Maps Binance `exchangeInfo` symbology into the shared `qv_model::Instrument` model, enforcing
...
use qv_model::{CurrencyPair, Instrument, InstrumentId, Symbol, Venue};
use qv_network::{
    Credentials, NetError, RateLimiter, RestClient, RestConfig, RestRequest,
};
use qv_ports::{InstrumentProvider, PortError, PortResult};
