//! [`BinanceDataClient`] — `impl qv_ports::DataClient`.
//!
//! Owns the market-data WebSocket and normalizes Binance frames into the venue-agnostic
//! `qv_model::MarketEvent`s, delivering them to the core over the single-consumer `mpsc` channel
//! taken once at wiring via [`take_stream`](qv_ports::DataClient::take_stream).
//!
...
//! `GET /api/v3/klines` and maps to `qv_model::Bar`. The wire-parsing is a set of PURE functions
...
use qv_ports::{DataClient, PortError, PortResult, SubKind, Subscription};
...
/// Map a `qv-network` transport error into a `qv-ports` error at the adapter boundary.
...
mod tests {
    ...
    fn kline_to_bar_uses_close_time_and_ohlcv() {
        let bar_type = BarType {
            ...
            spec: qv_model::BarSpec {
                step: 1,
                aggregation: BarAggregation::Minute,
                price_type: qv_model::PriceType::Last,
            },
            source: qv_model::AggregationSource::External,
        };
