//! [`BinanceExecutionClient`] — `impl qv_ports::ExecutionClient`, the live side of THE parity seam.
//!
//! Submits/cancels orders over signed REST and ingests acks/fills from the user-data WebSocket
//! stream, normalizing them into `qv_ports::ExecutionReport`s delivered over the `mpsc` channel
//! taken once via [`take_reports`](qv_ports::ExecutionClient::take_reports). The OMS/Risk above is
//! byte-for-byte identical to backtest (`qv-sim`'s `SimulatedExecutionClient`).
//!
...
/// Map a `qv-network` transport error into a `qv-ports` error at the adapter boundary. A 4xx with a
...
mod tests {
    use super::*;
    use qv_model::{OrderFlags, StrategyId};
