...
(`coinext-model`), behind three port traits:

| Port (`coinext-ports`)    | Sync? | What the adapter does                                                            |
|----------------------|-------|----------------------------------------------------------------------------------|
...
- **The mpsc seam.** Inbound streams are taken ONCE at Kernel wiring (`take_stream` /
  ...
  synchronous core over channels — the same shape `coinext-sim` uses in backtest. This is *why* the core
...
Adapters build on `coinext-network` (shared `WsClient` / `RestClient` / `RateLimiter`: reconnect,
...
2. Implement `DataClient`, `ExecutionClient`, `InstrumentProvider`, reusing `coinext-network`.
