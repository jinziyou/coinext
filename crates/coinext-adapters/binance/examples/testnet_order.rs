...
//!
//! ```bash
//! export COINEXT__BINANCE__API_KEY=...        # your SPOT testnet key
//! export COINEXT__BINANCE__API_SECRET=...     # your SPOT testnet secret
//! cargo run --manifest-path crates/coinext-adapters/binance/Cargo.toml --example testnet_order
//! # optional overrides:
//! #   COINEXT__ORDER__SYMBOL=BTCUSDT.BINANCE  COINEXT__ORDER__PRICE=20000  COINEXT__ORDER__QTY=0.001
//! ```
//!
...
use coinext_core::{Clock, Price, Quantity, SystemClock};
use coinext_model::{InstrumentId, OrderFlags, OrderSide, StrategyId, TimeInForce};
use coinext_ports::{CancelOrder, ExecutionClient, ExecutionReport, OrderFactory, SubmitOrder};
...
async fn main() {
    ...
    println!("=========================================================");
    println!("  Coinext — Binance SPOT TESTNET execution smoke test");
    ...
    if api_key.is_none() || api_secret.is_none() {
        eprintln!(
            "\nMissing credentials. Set COINEXT__BINANCE__API_KEY and COINEXT__BINANCE__API_SECRET\n\
