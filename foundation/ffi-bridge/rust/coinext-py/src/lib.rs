//! `coinext-py` — the ONLY in-process Python↔Rust binding (PyO3 + maturin).
//!
...
mod imp {
    ...
    use coinext_ports::{DataClient, RiskLimits, Strategy, StrategyContext, SubKind, Subscription};
    use pyo3::prelude::*;
    ...
    // --- Streaming technical indicators (the SAME Rust code as warm-up + live; see coinext-indicators).
    ...
    /// Native sandbox/live kernel handle returned by `build_kernel`.
    ///
    /// The handle is intentionally unsendable: the deterministic core owns `Rc<RefCell<_>>` state and
    /// must run on one thread. `run_with_portfolio_callback` executes the Rust live loop on a
    /// current-thread Tokio runtime and adapts every native `PortfolioSnapshot` into the Python
    /// `PortfolioSnapshot` class before invoking the supplied callback.
    #[pyclass(unsendable, name = "Kernel")]
    pub struct PyKernel {
        inner: RefCell<Option<LiveKernel>>,
        stop: LiveKernelStopHandle,
    }

    #[pymethods]
    impl PyKernel {
        /// The environment this native kernel targets: `"sandbox"` or `"live"`.
        #[getter]
        fn environment(&self) -> PyResult<String> {
            let kernel = self.inner.borrow();
            let Some(kernel) = kernel.as_ref() else {
                return Err(vexc("kernel is already running or unavailable"));
            };
            Ok(match kernel.environment() {
        ...
        /// Return the current native portfolio snapshot without publishing it.
        fn portfolio_snapshot(&self) -> PyResult<PyPortfolioSnapshot> {
            let kernel = self.inner.borrow();
            let Some(kernel) = kernel.as_ref() else {
                return Err(vexc("kernel is already running or unavailable"));
            };
            Ok(py_portfolio_snapshot(kernel.portfolio_snapshot()))
        }

        /// Request graceful shutdown of the native live loop.
        fn request_stop(&self) {
            self.stop.request_stop();
        }

        /// Alias for `request_stop`, convenient for Python lifecycle code.
        fn stop(&self) {
            self.stop.request_stop();
        }
        ...
        /// Run the native live loop without a Python telemetry callback.
        fn run(&self) -> PyResult<()> {
            let mut kernel = self.take_kernel()?;
            let rt = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .map_err(vexc)?;
            let local = tokio::task::LocalSet::new();
            let result = local.block_on(&rt, kernel.run());
            self.restore_kernel(kernel);
            result.map_err(vexc)
        }

        /// Run the native live loop and call `callback(PortfolioSnapshot)` after every mark/fill fold.
        fn run_with_portfolio_callback(&self, callback: Py<PyAny>) -> PyResult<()> {
            let mut kernel = self.take_kernel()?;
            let rt = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
            ...
            let result = local.block_on(&rt, async {
                kernel
                    .try_run_with_portfolio_callback(|snapshot| {
                        Python::attach(|py| {
                            let py_snapshot = Py::new(py, py_portfolio_snapshot(snapshot))?;
                            callback.bind(py).call1((py_snapshot,)).map(|_| ())
                        })
                    })
                    .await
            });
    ...
    impl PyKernel {
        fn take_kernel(&self) -> PyResult<LiveKernel> {
            self.inner
                .borrow_mut()
                .take()
                .ok_or_else(|| vexc("kernel is already running or unavailable"))
        }

        fn restore_kernel(&self, kernel: LiveKernel) {
            *self.inner.borrow_mut() = Some(kernel);
        }
    }
    ...
    pub struct PyResultObj {
        ...
        #[pyo3(get)]
        pub portfolio: PyPortfolioSnapshot,
    }
            ...
            for intent in intents {
                match intent {
                    ...
                    PyIntent::Cancel { client_order_id } => {
                        ctx.cancel(coinext_model::ClientOrderId::from(client_order_id.as_str()));
                    }
    ...
    impl Strategy for PyStrategyAdapter {
        ...
        fn on_quote(&mut self, q: &coinext_model::QuoteTick, ctx: &mut StrategyContext) {
            let v = (
        ...
        fn on_trade(&mut self, t: &coinext_model::TradeTick, ctx: &mut StrategyContext) {
            let v = (
        ...
        fn on_timer(&mut self, ev: &coinext_core::TimerEvent, ctx: &mut StrategyContext) {
    ...
    fn parse_environment(env: &str) -> PyResult<Environment> {
        match env.to_ascii_lowercase().as_str() {
            "sandbox" => Ok(Environment::Sandbox),
            "live" => Ok(Environment::Live),
            "backtest" => Ok(Environment::Backtest),
            other => Err(vexc(format!(
                "unknown kernel environment {other:?}; expected backtest, sandbox, or live"
            ))),
        }
    }

    fn py_attr<'py>(obj: &Bound<'py, PyAny>, name: &str) -> Option<Bound<'py, PyAny>> {
        obj.getattr(name).ok()
    }
    ...
    fn nested_attr<'py>(
        ...
        py_attr(obj, section).and_then(|section| py_attr(&section, field))
    }

    fn attr_string(obj: &Bound<'_, PyAny>, name: &str, default: &str) -> String {
        py_attr(obj, name)
            .and_then(|value| value.extract::<String>().ok())
            .unwrap_or_else(|| default.to_string())
    }

    fn nested_string(obj: &Bound<'_, PyAny>, section: &str, field: &str, default: &str) -> String {
        nested_attr(obj, section, field)
            .and_then(|value| value.extract::<String>().ok())
            .unwrap_or_else(|| default.to_string())
    }

    fn attr_f64(obj: &Bound<'_, PyAny>, name: &str, default: f64) -> f64 {
        py_attr(obj, name)
            .and_then(|value| value.extract::<f64>().ok())
            .unwrap_or(default)
    }

    fn nested_f64(obj: &Bound<'_, PyAny>, section: &str, field: &str, default: f64) -> f64 {
        nested_attr(obj, section, field)
            .and_then(|value| value.extract::<f64>().ok())
            .unwrap_or(default)
    }

    fn nested_u8(obj: &Bound<'_, PyAny>, section: &str, field: &str, default: u8) -> u8 {
        nested_attr(obj, section, field)
            .and_then(|value| value.extract::<u8>().ok())
            .unwrap_or(default)
    }

    fn nested_u32(obj: &Bound<'_, PyAny>, section: &str, field: &str, default: u32) -> u32 {
        nested_attr(obj, section, field)
            .and_then(|value| value.extract::<u32>().ok())
            .unwrap_or(default)
    }

    fn nested_bool(obj: &Bound<'_, PyAny>, section: &str, field: &str, default: bool) -> bool {
        nested_attr(obj, section, field)
    ...
    fn money_limit(value: f64, settle: Currency) -> PyResult<Option<Money>> {
        if value <= 0.0 {
            return Ok(None);
        }
        Money::from_decimal(Decimal::from_f64(value).unwrap_or(Decimal::ZERO), settle)
            .map(Some)
            .map_err(vexc)
    }
    ...
    fn run_kernel(
        ...
        if queue_ahead_factor > 0.0 {
            cfg.brokerage = Box::new(coinext_sim::DefaultBrokerageModel {
                queue_ahead_factor: Decimal::from_f64(queue_ahead_factor).unwrap_or(Decimal::ZERO),
                ..coinext_sim::DefaultBrokerageModel::default()
            });
        }
        ...
        let res = kernel.run();
        let portfolio = py_portfolio_snapshot(kernel.portfolio_snapshot());
    ...
    /// Build a native sandbox/live kernel from a structured Python `coinext_config.RunConfig`.
    ///
    /// Backtests still use `run_backtest` because they need an explicit historical event stream.
    /// Sandbox/live handles own real Binance data/execution clients and expose
    /// `run_with_portfolio_callback` so Python can publish native `PortfolioSnapshot` telemetry.
    #[pyfunction]
    #[pyo3(signature = (env, config, strategy=None))]
    pub fn build_kernel(
        env: String,
        ...
        if !env.is_live() {
            return Err(vexc(
                "coinext_py.build_kernel builds sandbox/live handles; use coinext_backtest.run for backtest",
            ));
        }
        let strategy = strategy.ok_or_else(|| {
            vexc("coinext_py.build_kernel requires a Python Strategy instance for sandbox/live")
        })?;

        let settle = Currency::new("USDT", 8).map_err(vexc)?;
        let symbol = attr_string(config, "symbol", "BTCUSDT");
        let venue = nested_string(config, "venue", "name", "BINANCE");
        let price_precision = nested_u8(config, "venue", "price_precision", 2);
        let size_precision = nested_u8(config, "venue", "size_precision", 3);
        let maker_fee = nested_f64(config, "venue", "maker_fee", 0.0002);
        let taker_fee = nested_f64(config, "venue", "taker_fee", 0.0004);
        let starting_balance = attr_f64(config, "starting_balance", 100_000.0);
        ...
        .map_err(vexc)?;
        let mut cfg =
            BacktestConfig::new(Venue::from(venue.as_str()), vec![inst], settle, starting);
        cfg.risk = RiskLimits {
            max_position_qty: None,
            max_position_notional: money_limit(
                nested_f64(config, "risk", "max_position_notional", 0.0),
                settle,
            )?,
            max_order_notional: money_limit(
                nested_f64(config, "risk", "max_order_notional", 0.0),
                settle,
            )?,
            max_orders_per_sec: {
                let v = nested_u32(config, "risk", "max_orders_per_sec", 0);
                if v == 0 {
                    None
                } else {
                    Some(v)
            ...
            max_gross_exposure: money_limit(
                nested_f64(config, "risk", "max_gross_exposure", 0.0),
                settle,
            )?,
            leverage: None,
        ...
        let default_testnet = matches!(env, Environment::Sandbox);
        let binance_config = coinext_adapters_binance::BinanceConfig {
            api_key: nonempty(nested_string(config, "binance", "api_key", "")),
            api_secret: nonempty(nested_string(config, "binance", "api_secret", "")),
            testnet: nested_bool(config, "binance", "testnet", default_testnet),
        };
        if !binance_config.has_credentials() {
            return Err(vexc(
                "coinext_py.build_kernel requires config.binance.api_key and api_secret for sandbox/live execution",
            ));
        }

        let mut data_client =
            coinext_adapters_binance::BinanceDataClient::new(binance_config.clone())
                .map_err(vexc)?;
        let exec_client =
            coinext_adapters_binance::BinanceExecutionClient::new(binance_config).map_err(vexc)?;
        ...
        rt.block_on(async {
            for kind in [
                SubKind::Quotes,
                SubKind::Trades,
                SubKind::BookL2 { depth: 1000 },
                SubKind::Bars(BarSpec {
                    step: 1,
                    aggregation: BarAggregation::Minute,
                    price_type: PriceType::Last,
                }),
    ...
    // --- Option pricing (Black–Scholes) — the SAME coinext-derivatives the Rust core uses. Inputs are
    ...
    fn bs_price(spot: f64, strike: f64, t_years: f64, rate: f64, vol: f64, is_call: bool) -> f64 {
        coinext_derivatives::price(&bs_inputs(spot, strike, t_years, rate, vol), is_call)
    }
    ...
    fn bs_greeks(
        ...
        let g = coinext_derivatives::greeks(&bs_inputs(spot, strike, t_years, rate, vol), is_call);
    ...
    fn implied_vol(
        ...
        coinext_derivatives::implied_vol(market_price, spot, strike, t_years, rate, is_call)
    }

    /// The `coinext_py` extension module.
    #[pymodule]
    fn coinext_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
        m.add_class::<PyBar>()?;
        m.add_class::<PyCtx>()?;
        m.add_class::<PyResultObj>()?;
        m.add_class::<PyPortfolioSnapshot>()?;
        ...
        m.add("__doc__", "Coinext Rust core exposed to Python (PyO3).")?;
        Ok(())
    }
}
