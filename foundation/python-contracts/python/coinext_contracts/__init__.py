...
LIVE_POSITION_PNL = "LivePositionPnl"  # payload["kind"] tag for live account/position telemetry


def _wire_scalar(value: object) -> object:
    """Return a JSON/MessagePack scalar, stringifying numerics so money keeps display precision."""
    if isinstance(value, (int, float)):
        return str(value)
    return value


def live_position_pnl_payload(
    *,
    account_id: str,
    environment: str,
    symbol: str,
    ...
    """Build the documented live position/PnL telemetry payload for ``coinext.live``."""
    return {
        ...
        "symbol": symbol,
        "venue": venue,
        "equity": _wire_scalar(equity),
        "gross_exposure": _wire_scalar(gross_exposure),
        "net_exposure": _wire_scalar(net_exposure),
        "realized_pnl": _wire_scalar(realized_pnl),
        "unrealized_pnl": _wire_scalar(unrealized_pnl),
...
def is_live_position_pnl(payload: dict) -> bool:
    """True if ``payload`` is a live position/PnL snapshot."""
