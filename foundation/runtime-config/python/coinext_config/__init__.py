...
SECTION_SEP = "__"
DEFAULT_CONFIG_DIR = "foundation/runtime-config/config"

# Known config environments (mirror DEFAULT_CONFIG_DIR environment YAML + base YAML).
...
def _yaml_layers(env: str, config_dir: str) -> dict[str, Any]:
    """Merge ``DEFAULT_CONFIG_DIR base YAML`` then ``foundation/runtime-config/config/{env}.yaml`` (env wins)."""
...
def _build_runconfig(merged: dict[str, Any]) -> RunConfig:
    ...
    ``DEFAULT_CONFIG_DIR base YAML``) onto the flat ``RunConfig`` + sub-model fields. Unknown keys are dropped
...
def load_config(
    ...
    *,
    ...
    Precedence (high to low): ``cli_overrides`` > env (``COINEXT__*``) > ``DEFAULT_CONFIG_DIR base/env YAML`` >
