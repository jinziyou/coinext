//! `coinext-ports` — ALL hexagonal port traits and their command/report value types in one place.
//!
//! Keeping ports together (and owning the `async-trait` dependency) resolves the port/value-type
//! crate-boundary muddle: `coinext-model` stays sync-only, the engines depend on these traits, and the
