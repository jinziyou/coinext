# risk-portfolio/risk-monitor/service/risk-monitor — out-of-band global risk supervisor
...
|-------------|-------------------------------------------------------------|
| Kind        | Python                                                       |
| Build       | `operations-interface/deployment/docker/risk-monitor.Dockerfile`                    |
...
```bash
docker build -f operations-interface/deployment/docker/risk-monitor.Dockerfile -t coinext/risk-monitor .
...
Telemetry payloads on `coinext.live` use the `LivePositionPnl` contract from `coinext_contracts`:
account equity, gross/net exposure, realized/unrealized PnL, and optional per-position rows. The
monitor folds those fields into `AccountState` before evaluating limits.

## Known gaps

- Export the Prometheus gauges/counters listed above.
- Add per-account/sub-account partitioning once multiple trader processes publish on the same stream.
