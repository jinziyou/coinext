...
# ----------------------------------------------------------------------------------------------
# ingestor (Rust qv-ingest) — market-data ingestion daemon.
#
...
# NOTE: qv-ingest is a workspace-excluded stub today (root Cargo.toml `exclude`); it is built here
...
COPY . .
# TODO(venue/IO): qv-ingest's real WS/REST ingestion lives in qv-network + qv-adapters/binance.
RUN cargo build --release --bin qv-ingest \
 && cp target/release/qv-ingest /qv-ingest

# --- runtime: distroless (no shell, minimal attack surface) ---
FROM gcr.io/distroless/cc-debian12 AS runtime
WORKDIR /app
COPY --from=builder /qv-ingest /app/qv-ingest

# Prometheus metrics endpoint (scraped by deploy/prometheus/prometheus.yml job `ingestor`).
EXPOSE 9101
# Config is supplied via VQ__* env (see .env / .env.example). The binary reads VQ__REDIS__URL,
# VQ__BINANCE__*, VQ__METRICS__PORT, VQ__OTEL__ENDPOINT, etc.
ENTRYPOINT ["/app/qv-ingest"]
CMD ["run"]
