"""execution-live/services/trader — thin per-account live process wrapper.

Status: scaffold. One process per account; builds coinext_live.TradingNode from COINEXT__* env.
Parity: same engines as backtest; only Clock/clients differ. See ARCHITECTURE.md §1/§7.
"""
...
def build_node(cfg: TraderConfig):
    ...
    its run loop.
    """
    from coinext_kernel import Environment  # noqa: WPS433 - lazy; only needed for live wiring

    coinext_live = _load_live()
    strategy = _build_strategy(cfg.strategy)
    node_cfg = coinext_live.TradingNodeConfig(
    ...
    return coinext_live.TradingNode(config=node_cfg, strategy=strategy)
...
def run(cfg: TraderConfig | None = None) -> None:
    ...
    try:
        import anyio  # noqa: WPS433 - live runtime dependency
    except ImportError as exc:  # pragma: no cover - environment-dependent
        raise RuntimeError(
            "anyio unavailable. Install the live extra before running the trader service."
        ) from exc
    anyio.run(node.run)
