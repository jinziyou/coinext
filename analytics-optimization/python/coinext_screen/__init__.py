...
# A vectorized result reduces to the SAME two surfaces a coinext_py BacktestResult exposes, so the parity
...
def to_session(result: VectorResult):
    """Convert a :class:`VectorResult` to a ``coinext_parity.SessionResult`` for the cross-check."""
    from coinext_parity import SessionResult
...
def _sharpe(equity_curve: list[tuple[int, float]]) -> float:
    ...
    if sd == 0.0:
        return 0.0
    # Annualized at minute-bar cadence (matches coinext_analytics).
...
def sweep_sma_cross(
    ...
    *,
    ...
    ``coinext_backtest.run`` (and check :func:`cross_check_vs_event`) before trusting any of them.
    """
...
def cross_check_vs_event(
    ...
    *,
    ...
    the same bars, then return ``coinext_parity.cross_check`` warnings (never raises).
    ...
    :func:`_snap_fills_to_grid`). Needs the compiled ``coinext_py`` for the event-driven leg.
    """
    import coinext_backtest
    from coinext_parity import SessionResult, cross_check
    ...
    event = SessionResult.from_backtest(
        coinext_backtest.run(SmaCross(fast=fast, slow=slow, qty=qty), symbol=symbol, bars=bars)
    )
