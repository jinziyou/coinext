...
//   - Kill-Switch         : GET /control/killswitch + /control/events, POST /control/killswitch
//
...
function KillSwitchPanel() {
  ...
  const events = useQuery<ControlEvent[]>({
    queryKey: ["control-events"],
    queryFn: () => api.getControlEvents(),
    refetchInterval: 1_000,
  });
  const killEvents = events.data?.filter((event) => event.kind === "CtrlKillSwitch");
  const latestKillEvent = killEvents?.[killEvents.length - 1];
  const projectedState: KillSwitchState | undefined = latestKillEvent
    ? {
        engaged: latestKillEvent.engaged,
        engaged_by: latestKillEvent.actor ?? latestKillEvent.source,
        reason: latestKillEvent.reason,
        ts_changed: latestKillEvent.ts_changed,
      }
    : state.data;
  ...
  const engaged = projectedState?.engaged ?? false;
  ...
  const mutation = useMutation({
    ...
    onSuccess: (next) => {
      ...
      void qc.invalidateQueries({ queryKey: ["control-events"] });
  ...
  return (
    <Panel
      title="Kill-Switch"
      isFetching={state.isFetching || events.isFetching}
      error={state.error ?? events.error}
      actions={
    ...
    >
      <div className="killswitch">
        ...
        {engaged && (projectedState?.reason || projectedState?.engaged_by) ? (
          <p className="muted small">
            Engaged by <strong>{projectedState?.engaged_by ?? "?"}</strong>
            {projectedState?.reason ? ` — “${projectedState.reason}”` : ""}
            {projectedState?.ts_changed ? ` (${fmtTs(projectedState.ts_changed)})` : ""}
          </p>
        ) : null}
