...
def encode_payload(payload: dict) -> bytes:
    """MessagePack-encode a payload map carried inside an Envelope."""
    msgpack = _require_msgpack()
    return msgpack.packb(payload, use_bin_type=True)


def decode_payload(env: Envelope) -> dict:
    """MessagePack-decode an Envelope's ``payload`` bytes into a dict payload map.
...
def build_payload_envelope(
    payload: dict,
    *,
    msg_type: MsgType = MsgType.CMD,
    trace_id: bytes | None = None,
) -> Envelope:
    """Build an Envelope carrying a MessagePack payload map."""
    tid = trace_id if trace_id is not None else os.urandom(16)
    return Envelope.of(msg_type, tid, time.time_ns(), encode_payload(payload))
...
def build_control_envelope(payload: dict, *, trace_id: bytes | None = None) -> Envelope:
    ...
    return build_payload_envelope(payload, msg_type=MsgType.CTRL, trace_id=trace_id)


class Publisher:
    """Thin cross-service publisher over :class:`RedisBusClient`.

    The API and risk-monitor publish ``CtrlKillSwitch`` commands through this, while trader processes
    publish live account/position telemetry on ``coinext.live``. Connections are opened lazily by the
    underlying client, so constructing a ``Publisher`` requires neither redis nor msgpack — they are
    needed only when a publish method is called.
    ...
    def publish_payload(
        ...
        *,
        ...
        """Wrap ``payload`` in an Envelope and publish it to ``stream``; return the message id."""
        env = build_payload_envelope(payload, msg_type=msg_type)
        return self.client.publish(stream, env)
    ...
    def publish_live_telemetry(
        ...
        *,
        ...
        """Publish a live account/position/PnL snapshot on ``coinext.live``."""
