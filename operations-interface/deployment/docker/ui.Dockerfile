...
#
# NOTE: the UI source lives under operations-interface/ui/service/ui/ (ARCHITECTURE.md §3). This is valid scaffolding; the
...
COPY operations-interface/ui/service/ui/package.json operations-interface/ui/service/ui/package-lock.json* ./
...
COPY operations-interface/ui/service/ui/ ./
...
# `api` service (so a same-origin VITE_API_BASE=/api avoids CORS). See operations-interface/ui/service/ui/nginx.conf.
COPY operations-interface/ui/service/ui/nginx.conf /etc/nginx/conf.d/default.conf
