...
//! loop (`coinext testnet-gate`). Reads a list of market orders, places each on testnet (paper), and
//! writes back the REAL fills, so the Python parity gate can diff testnet execution against the
//! deterministic backtest.
//!
//! IO (JSON):
//!   in  (COINEXT__ORDERS_IN,  default "orders.json"): `[{"side":"buy"|"sell","qty":0.001}, ...]`
//!   out (COINEXT__FILLS_OUT,  default "fills.json"):   `[{"ts":<ns>,"side":1|-1,"qty":..,"px":..}|{"error":".."}, ...]`
//!
//! Env: COINEXT__BINANCE__API_KEY / COINEXT__BINANCE__API_SECRET (spot testnet), COINEXT__ORDER__SYMBOL
//! (default BTCUSDT.BINANCE). Without keys it writes nothing and exits 2 (safe).
//!
//! Run: `cargo run --manifest-path market-data/venue-adapters/binance/rust/coinext-adapters-binance/Cargo.toml --example testnet_orders`

use coinext_adapters_binance::{BinanceConfig, BinanceExecutionClient};
use coinext_core::{Clock, Quantity, SystemClock};
use coinext_model::{InstrumentId, OrderSide, StrategyId};
use coinext_ports::{ExecutionClient, ExecutionReport, OrderFactory, SubmitOrder};
...
async fn main() {
    ...
    if api_key.is_none() || api_secret.is_none() {
        eprintln!(
            "testnet_orders: missing COINEXT__BINANCE__API_KEY / COINEXT__BINANCE__API_SECRET \
