...
```bash
# Build the image (Dockerfile builds market-data/ingestion-service/rust/coinext-ingest via its own manifest).
...
```bash
cargo run --release --manifest-path market-data/ingestion-service/rust/coinext-ingest/Cargo.toml
