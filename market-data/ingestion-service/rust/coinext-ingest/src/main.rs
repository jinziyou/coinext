...
//! live path with `cargo run --manifest-path market-data/ingestion-service/rust/coinext-ingest/Cargo.toml --features live`.
//!
...
//!   COINEXT__INGEST__SYMBOLS    comma-separated InstrumentIds (default "BTCUSDT.BINANCE,ETHUSDT.BINANCE").
//!   COINEXT__INGEST__MAX_EVENTS bounded demo: exit after N events (default 0 = run forever).
...
async fn run(
    ...
    #[cfg(feature = "live")]
    {
        ...
        println!("coinext-ingest: connected; streaming normalized events (Ctrl-C to stop)...");
        let mut count: u64 = 0;
        while let Some(ev) = rx.recv().await {
            ...
            // TODO(ingest): append to the data lake (coinext_persistence::ParquetWriter) and XADD a
            // versioned MessagePack Envelope to the Redis stream (coinext_bus); record
    ...
    #[cfg(not(feature = "live"))]
    {
        ...
        println!(
            "coinext-ingest: built client + {} subscriptions and took the event stream; \
