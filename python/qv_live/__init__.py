"""qv_live — the TradingNode (live / sandbox runtime).
...
The Binance clients live in Rust (``qv-adapters/binance``); this node only orchestrates lifecycle.
...
class TradingNodeConfig:
    """Live node wiring derived from a ``qv_config.RunConfig``."""
...
class TradingNode:
    ...
    Holds the strategy, the Kernel handle, the local :class:`~qv_data.HistoryReader` for warm-up,
    and (in a real build) the Rust-side Binance clients. ``run()`` is an async stub documenting the
    lifecycle; the actual I/O loops live on Tokio behind ``qv_py``.
    """

    config: TradingNodeConfig
    strategy: Any
    run_config: Any = None  # a qv_config.RunConfig
    ...
    def warmup(self) -> list[tuple[int, float]]:
        ...
        """
        from qv_data import BarSpec, HistoryReader  # local import: keeps qv_data optional at import
    ...
    def reconcile(self) -> dict[str, Any]:
        ...
        """
        # TODO: read append-only OrderEvent store (qv-persistence) + query Binance REST for open
    ...
    async def run(self) -> None:
        ...
        Tokio behind ``qv_py`` and only returns on shutdown / kill-switch.
        """
        if self.config.reconcile_on_start:
            self.reconcile()
        self.warmup()

        # TODO: self._kernel = qv_kernel.build_kernel(self.run_config, self.config.env)
        ...
        try:
            ...
            raise ImportError(
                "anyio not installed. Install the live extra: pip install 'veloxquant[live]'"
            ) from exc
        finally:
            self._running = False
