"""qv_bus — the Python Redis-Streams bus client.
...
Streams** bus and decodes the versioned **MessagePack** :class:`~qv_contracts.Envelope`
(``{schema_version, msg_type, trace_id, ts_init, payload}``). The ``ingestor`` (Rust ``qv-ingest``)
...
from qv_contracts import SCHEMA_VERSION, Envelope, MsgType

# Default stream keys on the bus (mirror the Rust publisher's keyspace).
STREAM_MARKET = "vq.market"  # quotes/trades/bars/deltas from the ingestor
STREAM_EXEC = "vq.exec"  # order events / fills from exec-svc
STREAM_CTRL = "vq.ctrl"  # control-plane commands (kill-switch, etc.)
...
def _require_redis() -> Any:
    try:
        ...
        raise ImportError(
            "redis not installed. Install the bus extra: pip install 'veloxquant[bus]'"
        ) from exc
    return redis
...
def _require_msgpack() -> Any:
    try:
        ...
        raise ImportError(
            "msgpack not installed. Install the bus extra: pip install 'veloxquant[bus]'"
        ) from exc
    return msgpack


def decode_envelope(raw: bytes) -> Envelope:
    """Decode a MessagePack frame into an :class:`~qv_contracts.Envelope`.

    The wire layout is a 5-element array ``[schema_version, msg_type, trace_id, ts_init, payload]``
    (compact, positional — matches the Rust ``qv_bus::Envelope`` serializer). Raises if the schema
...
def encode_envelope(env: Envelope) -> bytes:
    """Encode an :class:`~qv_contracts.Envelope` back to a MessagePack frame (symmetry helper)."""
    msgpack = _require_msgpack()
...
class RedisBusClient:
    ...
    def __init__(
        self, url: str = "redis://redis:6379/0", *, group: str = "qv", consumer: str = "c0"
    ) -> None:
        self.url = url
        self.group = group
    ...
    def publish(self, stream: str, env: Envelope) -> str:
        """Publish an :class:`~qv_contracts.Envelope` to ``stream`` (mainly for tests/tools)."""
        self.connect()
        assert self._client is not None
