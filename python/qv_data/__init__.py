"""qv_data — the data-lake catalog, history reader, and live data provider.

The data lake is a partitioned **Parquet** store on the local FS or S3/MinIO
(``VQ__DATA__LAKE_ROOT``, ``VQ__MINIO__*``). Partition layout (planned)::
...
# The Parquet lake (write/read/coverage) + paginated downloader. Guarded so `import qv_data` works
...
class DataCatalog:
    ...
    def __init__(self, lake_root: str | None = None) -> None:
        self.lake_root = lake_root or os.environ.get("VQ__DATA__LAKE_ROOT", "data")
...
class DataProvider:
    ...
    In live the analogous feed is the Redis-bus consumer (``qv_bus``); the Kernel sees the same
