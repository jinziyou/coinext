...
def detect_overfitting(metrics, trade_stats) -> list[str]:
    ...
    ``metrics`` is a ``coinext_analytics.Metrics``; ``trade_stats`` a ``coinext_analytics.trades.TradeStats``.
    """
    warnings: list[str] = []
    ts = trade_stats
...
def screen_biases(result, *, metrics=None, trade_stats=None, bars=None) -> BiasReport:
    """Run every bias screen against a ``coinext_py`` ``BacktestResult`` and bundle the warnings.
