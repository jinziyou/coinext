"""qv_contracts — the Python contract surface.

Re-exports the compiled domain/runtime types from ``qv_py`` (the source of truth), and defines the
MessagePack ``Envelope`` schema + ``MsgType`` registry that the Redis-Streams bus uses so the Rust
side and Python side (``qv_bus``) agree on the wire format. Port ``Protocol`` definitions describe
...
# Wire schema version — MUST match qv_bus::Envelope::SCHEMA_VERSION on the Rust side.
SCHEMA_VERSION = 1


class MsgType(IntEnum):
    """Payload kind tag in the Envelope (must match qv_ports::MsgType discriminants)."""
...
class Envelope:
    """Versioned cross-service frame. Mirrors qv_bus::Envelope."""
