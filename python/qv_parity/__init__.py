"""qv_parity — the pre-live promotion gate and the advisory cross-check.

VeloxQuant's core invariant is **backtest↔live parity** (see ``docs/ARCHITECTURE.md`` §1, §5): ONE
...
equity curve and a fills log, exactly what the compiled ``qv_py`` ``BacktestResult`` exposes.
"""
    ...
    * ``fills`` — list of ``(ts_ns, side, qty, px)`` where ``side`` is ``+1`` buy / ``-1`` sell.
    ...
    def from_fills_and_bars(
        ...
        *,
        ...
        Used by ``qv testnet-gate`` to build the SANDBOX session: the fills carry REAL testnet
...
def run_gate(
    ...
    **backtest_kwargs: Any,
    ...
    Run the authoritative event-driven backtest (``qv_backtest.run`` through the Rust kernel — the
    ...
    result = qv_backtest.run(strategy_factory(), bars=bars, **backtest_kwargs)
