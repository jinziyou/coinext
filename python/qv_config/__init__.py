"""qv_config — layered configuration for the VeloxQuant control plane.

Resolution order (highest precedence first), per ``docs/ARCHITECTURE.md`` §1/§7 + ``.env.example``::

    CLI flags  >  env (VQ__SECTION__KEY)  >  config/*.yaml files  >  built-in defaults

The same ``RunConfig`` is built for every :class:`~qv_kernel.Environment` — only the Kernel-injected
...
config file is actually read. ``from qv_config import load_config`` imports cleanly with NO heavy
deps installed.
"""
...
class BinanceConfig(BaseModel):
    ...
    # TODO: ws_base / rest_base overrides; recv_window; rate-limit weights per qv-network.
...
class VenueConfig(BaseModel):
    ...
    # TODO: slippage model params; partial-fill / queue-position fidelity knobs (qv-sim).


class RiskConfig(BaseModel):
    """Pre-trade risk gate + defense-in-depth limits (ARCHITECTURE.md §8, .env.example VQ__RISK__).
...
class RunConfig(BaseModel):
    ...
    postgres_dsn: str = Field(default="postgresql://veloxquant:veloxquant@postgres:5432/veloxquant")
...
def _env_layer() -> dict[str, Any]:
    """Project ``VQ__SECTION__KEY`` environment variables into a nested dict.

    ``VQ__RISK__MAX_ORDERS_PER_SEC=20`` -> ``{"risk": {"max_orders_per_sec": "20"}}``.
    ``VQ__ENV=live`` -> ``{"env": "live"}`` (no section).
    """
    out: dict[str, Any] = {}
...
def load_config(
    ...
    *,
    ...
    Precedence (high to low): ``cli_overrides`` > env (``VQ__*``) > ``config/{base,env}.yaml`` >
    code defaults. ``env`` defaults to ``$VQ__ENV`` or ``"backtest"``.

    The CLI (``qv_cli``) passes already-parsed flags as ``cli_overrides`` (a nested dict mirroring
