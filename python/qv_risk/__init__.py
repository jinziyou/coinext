"""qv_risk — Python-side risk config facade + protections pipeline.

The AUTHORITATIVE pre-trade gate is the Rust ``qv-risk-engine`` (a synchronous gate + atomic
kill-switch, ARCHITECTURE.md §8). This package is the Python-side facade: it owns the *config* the
Rust gate reads (``VQ__RISK__*``) and a defense-in-depth **protections pipeline** modelled on
...
@dataclass
class RiskLimits:
    """Per-order / portfolio limits (mirrors ``qv_config.RiskConfig`` and ``VQ__RISK__*``).
...
class PortfolioSnapshot:
    """Minimal portfolio view the protections need (filled from ``qv_portfolio`` / the bus)."""
