...
class Ctx(Protocol):
    ...
    def submit_stop_limit(
        self, side: str, qty: float, trigger: float, price: float, symbol: str | None = None
    ) -> str: ...
    def submit_trailing(
        self, side: str, qty: float, offset: float, symbol: str | None = None
    ) -> str: ...
...
class _Sma:
    """Tiny streaming SMA (mirrors coinext-indicators::Sma) so the example is self-contained."""
...
class RsiReversion(Strategy):
    ...
    Uses the SHARED Rust ``coinext_indicators.Rsi`` — the identical incremental implementation the
    native-Rust path and live warm-up use, not a re-rolled Python copy. The import is lazy (in
    ``__init__``) so ``coinext_strategy`` still imports without the compiled ``coinext_py`` present.
    """

    def __init__(
        self, period: int = 14, low: float = 30.0, high: float = 70.0, qty: float = 0.5
    ) -> None:
        from coinext_indicators import Rsi
