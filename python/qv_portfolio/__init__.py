"""qv_portfolio — Python facade mirroring the Rust ``Portfolio`` port.

The AUTHORITATIVE portfolio (balances, realized/unrealized PnL, exposure) lives in Rust
(``qv-portfolio``), sourced from the Cache marks (ARCHITECTURE.md §3, §7). This package is a read
...
``qv_py`` in-process or from the Redis bus out-of-process.
...
class Portfolio:
    ...
    A real instance is fed by ``qv_py`` (in-process) or reconstructed from bus events (out-of-proc).
    ...
    def from_native(cls, native: Any) -> Portfolio:
        """Build a facade from a ``qv_py`` portfolio handle.

        TODO: map the native Position/Account objects (integer domain) into the float views here.
        """
        raise NotImplementedError(
            "from_native is a stub; wire to qv_py's Portfolio export once available."
        )
