...
``coinext_py.run_backtest`` (the SAME engines + SimulatedExecutionClient the live path uses). A separate
...
def run(
    ...
    *,
    ...
    return coinext_py.run_backtest(
        strategy,
        symbol,
        venue,
        starting_balance,
...
def run_multi(
    ...
    *,
    ...
    return coinext_py.run_backtest_multi(
...
def synth_trades(bars: list[tuple]) -> list[tuple]:
    ...
    use ``coinext_data.fetch_binance_agg_trades`` for genuine per-print microstructure.
    """
    out: list[tuple] = []
