"""qv_derivatives — European option pricing (Black–Scholes) + greeks + implied vol.

Thin wrappers over the compiled Rust ``qv-derivatives`` (via ``qv_py``), so a Python strategy prices
...
Inputs are decimals (``rate=0.05``, ``vol=0.2``, ``t_years=0.5``); ``right`` is call or put.
    ...
    from qv_derivatives import bs_price, greeks, implied_vol
...
def bs_price(
    ...
    return qv_py.bs_price(spot, strike, t_years, rate, vol, _is_call(right))
...
def greeks(
    ...
    return Greeks(*qv_py.bs_greeks(spot, strike, t_years, rate, vol, _is_call(right)))
...
def implied_vol(
    ...
    return qv_py.implied_vol(market_price, spot, strike, t_years, rate, _is_call(right))
